"""Reference safe adapters for AEON Phase B.1.

GitHub Coder should merge these semantics into the current adapters.py rather
than blindly replacing unrelated future changes.
"""
...
def createResponseSeries(sessionContext: dict[str, Any]) -> dict[str, Any]:
    required = ("series_id", "session_id", "participant_id", "protocol_id", "started_at")
    missing = [field for field in required if not sessionContext.get(field)]
    if missing:
        raise ValueError(
