...
from validator import validate_instance, ProtocolValidationError
from measurement_semantic_validator import ensure_measurement_semantics

ROOT = Path(__file__).resolve().parent
EXAMPLES = ROOT / "examples"

class MeasurementReferenceNotFoundError(LookupError): pass
class MeasurementReferenceDuplicateError(LookupError): pass

def _load(pattern: str, field: str, identifier: str) -> dict[str, Any]:
    matches=[]
    for path in sorted(EXAMPLES.rglob(pattern)):
        value=json.loads(path.read_text(encoding="utf-8"))
        if value.get(field)==identifier: matches.append(value)
    if not matches: raise MeasurementReferenceNotFoundError(identifier)
    if len(matches)>1: raise MeasurementReferenceDuplicateError(identifier)
...
def createResponseSeriesV02Draft(context: dict[str, Any]) -> dict[str, Any]:
    value={"schema_version":"0.2-draft","series_id":context["series_id"],"session_id":context["session_id"],"participant_id":context["participant_id"],"protocol_id":context["protocol_id"],"started_at":context["started_at"],"observations":[],"engine_records":[],"derived_metrics":[],"behavioral_records":[],"interpretation_boundary":"Behavioral records describe observed choices and timing; they do not diagnose, establish causality, or prove symbolic constructs."}
